@rem @meta description=Unblocks file on the selected directory
@rem @meta usage=unblock [dir]
@echo off
setlocal enabledelayedexpansion

:: Check if path argument is provided, otherwise use current directory
if "%~1"=="" (
    set "targetPath=%CD%"
) else (
    set "targetPath=%~1"
...
if not exist "%targetPath%" (
    echo Error: Path does not exist: %targetPath%
    pause
    exit /b 1
)
...
del "%tempFile%" >nul 2>&1
echo.
echo Completed unblocking %count% files.
pause